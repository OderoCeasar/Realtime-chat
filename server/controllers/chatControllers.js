...
const removeContact = async (req, res) => {
    ...
    try {
        const user1 = await user.findByIdAndUpdate(currentUserId, {
            $pull: { contacts: userId }
        }, { new: true });

        const user2 = await user.findByIdAndUpdate(currentUserId, {
            $pull: { contacts: userId }
        }, { new: true });

        res.status(200).send({ message: "Contact removed", user1, user2 })
    } catch (error) {
        res.status(500).send(error);
    }
};
