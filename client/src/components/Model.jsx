...
function Model(props) {
    ...
    const updateBtn = async () => {
        if (name) {
            let data = await renameGroup({ chatId: activeChat._id, userId: Element._id })
            if (res._id) {
        ...
        return (
            <div>
                ...
                <button onClick={handleOpen}>
                    <img className="w-[40px] h-[40px] rounded-[25px]" src={getChatPhoto(activeChat, activeUser)} alt="Profile Pic" />
                </button>
                {
                    activeChat?.isGroup ?
                            ...
                            <div>
                                        ...
                                        members.length > 0 && members?.map((e) => {
                                            return (
                                                <button button className="flex items-center gap-x-1 bg-green-100 text-green-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-green-400 border border-green-400">
                                                    <span className="text-[10px]">{e._id === activeUser._id ? "You" : e.name}</span>
                                ...
                                <div>
                                    <form className="mt-5 flex flex-col gap-y-3" onSubmit={(e) => e.preventDefault}>
                                        <input onChange={(e) => setName(e.target.value)} value={name} className="border-[#c4ccdf] border-[1px] text-[13.5px] py-[4px] px-2 w-[100%]" type="text" name="chatName" placeholder="Group Name" required />
